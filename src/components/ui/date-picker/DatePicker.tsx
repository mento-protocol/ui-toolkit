...
const DatePickerButton: React.FC<DatePickerButtonProps> = ({
  ...
  return (
    <Popover.Button
      ref={closeButtonRef}
      className={cn(
        "w-full rounded-[4px] border border-input p-1 text-left",
        className,
      )}
    >
      {children}
  ...
  return (
    <Popover onAbort={onClose} className={cn(className, "relative h-full")}>
      {({ close }) => (
        <>
          {buttonChild}
          <Popover.Panel className="absolute right-0 z-10 flex w-[300px] flex-col items-center rounded-[4px] border border-input bg-background">
            <Calendar
              {...restProps}
